...
def typetest(mv, utm):
    ...
    if utm == True:
        ...
        while time.time()-test_start_time < mv: # Runs loop while duration (mv) is not exceeded
            ...
            startinput_time = time.time() # Stores time at input request
            topress = random.choice(pool) # selects a random letter to type

            print('Type letter '+ Fore.LIGHTBLUE_EX + topress + Fore.WHITE)

            keypress = readkey()# Reads pressed key
            finishinput_time = time.time() - startinput_time # Stores time of input duration

            if ord(keypress) == 32: # If spacebar is pressed, the program stops

                break

            elif ord(keypress) == ord(topress): # If the correct key is pressed, it will be shown in green
                ...
                print("You typed letter " + Fore.RED + str(keypress) + Fore.WHITE) # If the wrong key is pressed, it will be shown in red
        ...
        for i in range(mv):
            ...
            topress = random.choice(pool)

            print('Type letter '+ Fore.LIGHTBLUE_EX + topress + Fore.WHITE)

            keypress = readkey()
            finishinput_time = time.time() - startinput_time

            if ord(keypress) == 32:

                break

            elif ord(keypress) == ord(topress):
